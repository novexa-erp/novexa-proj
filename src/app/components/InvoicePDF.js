    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 36 }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 13, color: "#6b7280", marginTop: 6, fontWeight: 600 }}>
            {inv.invoiceNumber || InvoiceNumber(inv.id)}
          </div>
      ...
      <div style={{ position: "absolute", bottom: 36, left: 56, right: 56 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#9ca3af" }}>
          ...
          <span>{inv.invoiceNumber || InvoiceNumber(inv.id)}</span>
...
export default function InvoicePDFModal({ inv, userDoc, onClose, payments = [], customerTotalBalance = null }) {
  ...
  async function shareWhatsApp() {
    ...
    const num  = inv.invoiceNumber || InvoiceNumber(inv.id);
  ...
  async function copyLink() {
    const text = `Invoice ${inv.invoiceNumber || InvoiceNumber(inv.id)} | ${inv.customerName || inv.customer} | Total: ${formatRs(inv.amount)} | Balance: ${formatRs(inv.balance || 0)}`;
    try {
      ...
      <div className="w-full max-w-[860px] my-4">
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4 px-1">
          <h3 className="text-white font-bold text-sm sm:text-base">
            Invoice Preview — {inv.invoiceNumber || InvoiceNumber(inv.id)}
          </h3>
